...
export default class Secant extends Equations{
  ...
  xChange(event:ChangeEvent<HTMLInputElement>){
      this.props.StateEquation.Method.RootEquations.Secant.x = JSON.parse(event.target.value);
      this.setState({StateEquation: this.props.StateEquation});
  }
  xiChange(event:ChangeEvent<HTMLInputElement>){
    this.props.StateEquation.Method.RootEquations.Secant.xi = JSON.parse(event.target.value);
    this.setState({StateEquation: this.props.StateEquation});
}
  ...
  handleSubmit(event:FormEvent<HTMLFormElement>){
      ...
      let Result:any = this.calc(
        this.state.StateEquation.Method.RootEquations.Secant.x,
        this.state.StateEquation.Method.RootEquations.Secant.xi,
  ...
  async componentDidMount() {
      ...
      await axios.get(api, { headers: {"Authorization" : `Bearer ${this.props.StateEquation.Token}`} })
          .then(res => {
              ...
              console.log(res.data.Chapter[4].Secant);
              this.state.StateEquation.Problem = res.data.Chapter[4].Secant;
              this.setState({StateEquation:this.props.StateEquation})
          });
    }
    render(){
            ...
            <NavBar />
              ...
              <form onSubmit={this.handleSubmit}>
                <div className="myform">
                  ...
                  <TextField id="demo-helper-text-misaligned" label="X" type={"number"} defaultValue={this.state.StateEquation.Method.RootEquations.Secant.x} inputProps={{step: Math.pow(10,-6)}} onChange={this.xChange}/>
                  <TextField id="demo-helper-text-misaligned" label="Xi" type={"number"} defaultValue={this.state.StateEquation.Method.RootEquations.Secant.xi} inputProps={{step: Math.pow(10,-6)}} onChange={this.xChange}/>
