...
export default class Newtonraphson extends Equations{
  ...
  constructor(props:PropNumerical) {
    ...
    xChange(event:ChangeEvent<HTMLInputElement>){
      this.props.StateEquation.Method.RootEquations.Onepoint.x = JSON.parse(event.target.value);
    ...
    handleSubmit(event:FormEvent<HTMLFormElement>) {
        ...
        let Result:any = this.calc(
            this.state.StateEquation.Method.RootEquations.Newton.x,
    ...
    async componentDidMount() {
      ...
      await axios.get(api, { headers: {"Authorization" : `Bearer ${this.props.StateEquation.Token}`} })
          .then(res => {
              ...
              console.log(res.data.Chapter[3].NewtonRaphson);
              this.state.StateEquation.Problem = res.data.Chapter[3].NewtonRaphson;
              this.setState({StateEquation:this.props.StateEquation})
          });
    }
    render(){
            ...
            <NavBar />
              ...
              <form onSubmit={this.handleSubmit}>
                <div className="myform">
                  ...
                  <TextField id="demo-helper-text-misaligned" label="X" type={"number"} defaultValue={this.state.StateEquation.Method.RootEquations.Newton.x} inputProps={{step: Math.pow(10,-6)}} onChange={this.xChange}/>
